...
function isValue(arg) {
  return !(
    isOption(arg) ||
    re.combinedShort.test(arg) ||
    re.optEquals.test(arg)
  );
}
...
class OptionDefinition {
  ...
  isBoolean() {
    return (
      this.type === Boolean ||
      (t.isFunction(this.type) && this.type.name === "Boolean")
    );
  }
...
class Definitions extends Array {
  ...
  validate(caseInsensitive) {
    ...
    const someDontHaveFunctionType = this.some(
      (def) => def.type && typeof def.type !== "function"
    );
    ...
    if (numericAlias) {
      halt(
        "INVALID_DEFINITIONS",
        "Invalid option definition: to avoid ambiguity an alias cannot be numeric [--" +
          invalidOption.name +
          " alias is -" +
          invalidOption.alias +
          "]"
    ...
    const duplicateName = hasDuplicates(
      this.map((def) => (caseInsensitive ? def.name.toLowerCase() : def.name))
    );
    if (duplicateName) {
      halt(
        "INVALID_DEFINITIONS",
    ...
    const duplicateAlias = hasDuplicates(
      this.map((def) =>
        caseInsensitive && t.isDefined(def.alias)
          ? def.alias.toLowerCase()
          : def.alias
      )
    );
    if (duplicateAlias) {
    ...
    const duplicateDefaultOption =
      this.filter((def) => def.defaultOption === true).length > 1;
    ...
    if (isOption(arg)) {
      if (re.short.test(arg)) {
        ...
        if (caseInsensitive) {
          ...
          return this.find(
            (def) =>
              t.isDefined(def.alias) &&
              def.alias.toLowerCase() === lowercaseShortOptionName
          );
        } else {
        ...
        if (caseInsensitive) {
          ...
          return this.find(
            (def) => def.name.toLowerCase() === lowercaseOptionName
          );
        } else {
  ...
  static from(definitions, caseInsensitive) {
    ...
    const result = super.from(arrayify(definitions), (def) =>
      OptionDefinition.create(def)
    );
...
class ArgvParser {
  ...
  constructor(definitions, options) {
     ...
     * Option Definitions
     */
    this.definitions = Definitions.from(
      definitions,
    ...
    if (this.argv.hasCombinedShortOptions()) {
      findReplace(
        this.argv,
        re.combinedShort.test.bind(re.combinedShort),
        (arg) => {
          arg = arg.slice(1);
          return arg
            .split("")
            .map((letter) => ({ origArg: `-${arg}`, arg: "-" + letter }));
        }
      );
    }
  }
  ...
  _set(val, state) {
    ...
    if (def.isMultiple()) {
      ...
      if (!def.isMultiple() && this.state === "set") {
        const err = new Error(
          `Singular option already set [${this.definition.name}=${this.get()}]`
        );
...
function commandLineArgs(optionDefinitions, options) {
  ...
  optionDefinitions = Definitions.from(
    optionDefinitions,
